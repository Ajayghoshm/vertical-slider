...
const linearInterpolator = {
  ...
  getValueForClientX: (clientX, trackDims, min, max) => {
    const { top, height } = trackDims;
    console.debug("height", height);
    const percentageValue = (clientX - top) / height;
  ...
  const handleKeyDown = React.useCallback(
    (e, i) => {
      ...
      if (e.keyCode === 37 || e.keyCode === 39) {
        ...
        console.debug("values", sortedValues);
    ...
    return ticks.map((value, i) => ({
      ...
      getTickProps: ({ key = i, style = {}, ...rest } = {}) => ({
        ...
        style: {
          position: "absolute",
          width: 0,
          top: `${getPercentageForValue(value)}%`,
      ...
      (tempValues || values).map((value, i) => ({
        ...
        getHandleProps: ({
          ...
          style: {
            position: "absolute",
            top: `${getPercentageForValue(value)}%`,
            zIndex: i === activeHandleIndex ? "0" : "1",
